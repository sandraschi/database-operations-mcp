...
async def test_tool_execution(mocker):
    ...
    client = TestClient(app)
    response = client.post("/tools/call", json={"name": "test_tool", "args": {"param1": "value1"}})
...
def test_nonexistent_tool():
    ...
    client = TestClient(app)
    response = client.post("/tools/call", json={"name": "nonexistent_tool", "args": {}})
...
def test_invalid_tool_request():
    ...
    response = client.post(
        "/tools/call", content="invalid json", headers={"Content-Type": "application/json"}
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
async def test_tool_execution_error(mocker):
    ...
    client = TestClient(app)
    response = client.post("/tools/call", json={"name": "failing_tool", "args": {}})

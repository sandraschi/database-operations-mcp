...
import inspect
import asyncio
from typing import Dict, List, Any, Optional, Type, get_type_hints, TypeVar, Callable
...
class FastMCPHelpTool(FastMCPTool):
    ...
    async def _get_tool_help(self, tool_name: str) -> Dict[str, Any]:
        ...
        tool_def = tool.definition if hasattr(tool, 'definition') else {
    ...
    async def _list_tools(
        ...
        if search_term:
            ...
            tools = {
                ...
                if (search_term in name.lower() or 
                    search_term in getattr(tool, 'description', '').lower() or
                    search_term in getattr(tool, '__doc__', '').lower())
            }
        ...
        result = {
            'tools': [
                await self._get_tool_help(name) if detailed
                else {
                    'name': name,
                    'description': getattr(tool, 'description', 'No description available.'),
                }

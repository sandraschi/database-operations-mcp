    ...
    async def start(self) -> None:
        ...
        try:
            ...
            writer_transport, writer_protocol = await loop.connect_write_pipe(
                asyncio.streams.FlowControlMixin, asyncio.subprocess.PIPE
            )
            ...
            while self.running:
                try:
                    ...
                    try:
                        ...
                        error = self._create_error_response(None, -32700, "Parse error", str(e))
                        writer.write((json.dumps(error) + "\n").encode("utf-8"))
                        await writer.drain()

                    except Exception as e:
                        logger.error(f"Error processing request: {e}", exc_info=self.debug)
                        error = self._create_error_response(None, -32603, "Internal error", str(e))
                        writer.write((json.dumps(error) + "\n").encode("utf-8"))
                        await writer.drain()

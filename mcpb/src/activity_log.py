...
_max_entries = max(
    100,
    min(int(os.environ.get("DBOPS_LOG_MAX_ENTRIES", str(_DEFAULT_MAX))), 50_000),
)
_lock = Lock()
...
def get_activity(limit: int = 50) -> list[dict[str, Any]]:
    result = query_logs(limit=limit, offset=0, sort="desc")
...
def export_logs(
    *,
    ...
    if format == "csv":
        ...
        return buffer.getvalue(), "text/csv", f"database-operations-mcp-logs-{stamp}.csv"

    body = json.dumps(
        {"exported_at": datetime.now(UTC).isoformat(), "entries": entries},
        indent=2,
    )
    return body, "application/json", f"database-operations-mcp-logs-{stamp}.json"

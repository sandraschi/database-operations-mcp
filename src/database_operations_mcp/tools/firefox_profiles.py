...
async def firefox_profiles(
    ...
    Operations: get_firefox_profiles, create_firefox_profile, delete_firefox_profile,
    create_loaded_profile, create_portmanteau_profile, suggest_portmanteau_profiles,
    create_loaded_profile_from_preset, check_firefox_status.

    Requires Firefox to be closed for profile create/delete operations.

    Args:
        operation: Operation to perform (required)
        profile_name: Target profile name
        source_profiles: List of source profiles for portmanteau/loaded ops
        preset_name: Preset for create_loaded_profile_from_preset
            (developer_tools, ai_ml, cooking, productivity, etc.)
        profile_config: Config dict for create ops
        check_status: Verify Firefox closed. Default: True
        include_bookmarks: Copy bookmarks from source. Default: True
        include_settings: Copy settings from source. Default: True
    """

...
from .utils import get_profiles_ini_path, parse_profiles_ini, get_profile_directory
from .curated_sources import get_curated_source, list_curated_sources, CURATED_SOURCES
...
class FirefoxStatusChecker:
                ...
                try:
                    if 'firefox' in proc.info['name'].lower():
                        firefox_processes.append({
                            'pid': proc.info['pid'],
                            'name': proc.info['name'],
                            'cmdline': proc.info['cmdline'][:3] if proc.info['cmdline'] else []  # First 3 args only
                        })
    ...
    def check_database_access_safe(profile_path: Optional[Path] = None) -> Dict[str, Any]:
        ...
        return {
            'safe': True,
            'message': "Safe to access Firefox databases",
            'details': status
        }
...
async def check_firefox_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "recommendation": "Close Firefox completely before running bookmark operations" if status['is_running'] else "Firefox is closed - safe to access bookmark databases"
    }
...
async def create_firefox_profile(
    ...
    try:
        ...
        if not profiles_ini_path:
            return {
                "status": "error",
                "message": "Could not locate Firefox profiles.ini file"
            }
...
@mcp.tool()
@HelpSystem.register_tool(category='firefox')
async def delete_firefox_profile(profile_name: str, confirm_deletion: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if profiles_ini_path and profiles_ini_path.exists():
            ...
            for section in config.sections():
                if (section.startswith('Profile') and
                    config[section].get('Name') == profile_name):
...
async def create_loaded_profile(
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Failed to create loaded profile: {str(e)}"
        }

async def _fetch_bookmarks_from_source(source_type: str, config: Dict[str, Any], max_bookmarks: int) -> List[Dict[str, str]]:
...
async def _fetch_from_current_collection(config: Dict[str, Any], max_bookmarks: int) -> List[Dict[str, str]]:
...
async def _fetch_from_github_awesome(config: Dict[str, Any], max_bookmarks: int) -> List[Dict[str, str]]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(search_url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                ...
                for repo in repos:
                    ...
                    try:
                        async with session.get(readme_url, timeout=aiohttp.ClientTimeout(total=5)) as readme_response:
                            if readme_response.status == 200:
                                ...
                                matches = re.findall(link_pattern, readme_content)[:max_bookmarks//len(repos)]
...
async def _populate_profile_with_bookmarks(profile_name: str, bookmarks: List[Dict[str, str]]) -> int:
...
async def create_portmanteau_profile(
    ...
    try:
        ...
        if not valid_presets:
            return {
                "status": "error",
                "message": "No valid presets specified"
            }
        ...
        return {
            "status": "error",
            "message": f"Failed to create portmanteau profile: {str(e)}"
        }

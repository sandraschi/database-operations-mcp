...
class PlexDatabase:
    ...
    def _locate_database(self, db_path: Optional[str] = None) -> Path:
        ...
        raise FileNotFoundError(
            "Could not locate Plex database. Please specify the path manually."
        )
    ...
    def get_media_items(self, section_id: Optional[int] = None, 
    ...
    def export_library(self, output_format: str = 'json', 
...
def export_plex_library(db_path: Optional[str] = None, 
...
def register_tools(mcp):
    ...
    @mcp.tool()
    def get_plex_library_sections(db_path: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            with PlexDatabase(db_path) as plex_db:
                return {
                    'status': 'success',
                    'sections': plex_db.get_library_sections()
                }
        except Exception as e:
            return {
                'status': 'error',

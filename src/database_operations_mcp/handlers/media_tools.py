...
def register_tools(mcp):
    ...
    mcp.tool()(find_plex_database)
    mcp.tool()(optimize_plex_database)
    mcp.tool()(export_database_schema)
...
@HelpSystem.register_tool(category='export')
@mcp_tool()
async def export_database_schema(
    ...
    try:
        ...
        if output_format == 'sql':
            result = _export_to_sql(cursor, tables, output_file, include_data)
        elif output_format == 'json':
            result = _export_to_json(cursor, tables, output_file, include_data)
        elif output_format == 'csv':
            result = _export_to_csv(cursor, tables, output_file, include_data)
        else:
...
def _export_to_sql(cursor, tables: List[str], output_file: str, include_data: bool) -> None:
...
def _export_to_json(cursor, tables: List[str], output_file: str, include_data: bool) -> None:
...
def _export_to_csv(cursor, tables: List[str], output_dir: str, include_data: bool) -> None:
    ...
    for table in tables:
        ...
        with open(Path(output_dir) / f"{table}.csv", 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)

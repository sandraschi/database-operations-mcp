"""Chrome/Chromium bookmark handling via JSON files."""
...
class ChromeBrowser:
    """Chrome browser bookmark manager."""

    def __init__(self) -> None:
        """Initialize Chrome browser handler."""
    ...
    def _find_bookmark_file(self) -> Path | None:
        ...
        if os.name == "nt":  # Windows
            ...
            if appdata:
                return Path(appdata) / "Google" / "Chrome" / "User Data" / "Default" / "Bookmarks"
        else:  # macOS/Linux
            home = Path.home()
    ...
    async def get_bookmarks(self, folder: str | None = None) -> list[dict[str, Any]]:
        ...
        if not self.bookmark_file or not self.bookmark_file.exists():
            logger.error("Chrome bookmarks file not found")
            return []
...
async def get_chrome_bookmarks() -> list[dict[str, Any]]:
    """Get Chrome bookmarks."""
    browser = ChromeBrowser()
    return await browser.get_bookmarks()


async def add_chrome_bookmark(url: str, title: str, folder: str | None = None) -> dict[str, Any]:
    """Add bookmark to Chrome."""
    browser = ChromeBrowser()

"""Brave browser bookmark handling."""
...
class BraveBrowser:
    """Brave browser bookmark manager."""

    def __init__(self) -> None:
        """Initialize Brave browser handler."""
    ...
    async def get_bookmarks(self, folder: str | None = None) -> list[dict[str, Any]]:
        """Get bookmarks from Brave (uses Chrome format)."""
        if not self.bookmark_file or not self.bookmark_file.exists():
            logger.error("Brave bookmarks file not found")
            return []
        ...
        try:
            ...
            logger.error(f"Error reading Brave bookmarks: {e}", exc_info=True)
            return []
...
async def get_brave_bookmarks() -> list[dict[str, Any]]:
    """Get Brave bookmarks."""
    browser = BraveBrowser()
    return await browser.get_bookmarks()


async def add_brave_bookmark(url: str, title: str, folder: str | None = None) -> dict[str, Any]:
    """Add bookmark to Brave."""
    browser = BraveBrowser()

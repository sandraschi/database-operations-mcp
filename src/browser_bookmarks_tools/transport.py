...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "10708")),
...
""",
    ...
    parser.add_argument(
        "--port", type=int, default=None, help=f"Port to listen on (default: ${ENV_PORT} or 10708)"
    )
    parser.add_argument(
...
def run_server(
    ...
    try:
        if transport == "stdio":
            ...
            mcp_app.run()  # stdio is default in FastMCP 2.14+

        elif transport == "http":
            # HTTP Streamable mode (FastMCP 2.14.4+)
            host = config["host"]
            port = config["port"]
            path = config["path"]
            endpoint = f"http://{host}:{port}{path}"
            logger.info(f"Running in HTTP Streamable mode: {endpoint}")
            mcp_app.run(transport="http", host=host, port=port, path=path)
        ...
        logger.info(f"{server_name} shutdown requested by user")

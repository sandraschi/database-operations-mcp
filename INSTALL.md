...
4. Start the server:
   ...
   uv run python -m database_operations_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn database_operations_mcp.server:app --port 10708
   ```
5. Open `http://localhost:10708` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/database-operations-mcp/issues) |

...
class ToolTester:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if 'from ...' in stripped or 'from ....' in stripped:
                    analysis["import_issues"].append({
                        "line": i,
                        "issue": "Relative import may cause issues",
                        "text": stripped
                    })
    ...
    def analyze_fastmcp_compliance(self):
        ...
        for module_name, module_result in self.results["modules"].items():
            ...
            if analysis["indentation_issues"]:
                for issue in analysis["indentation_issues"]:
                    issues.append(
                        f"{module_name} line {issue['line']}: {issue['issue']}"
                    )
        ...
        self.results["fastmcp_compliance"] = {
            "issues": issues,
    ...
    def generate_report(self):
        ...
        if compliance["issues"]:
            report_lines.extend([
                f"Found {len(compliance['issues'])} compliance issues:",
                ""
            ])
        ...
        for module_name, module_result in self.results["modules"].items():
            ...
            if module_result["import_success"]:
                report_lines.append(f"  ✅ Import: SUCCESS")
                ...
                report_lines.append(f"  ❌ Import: FAILED")

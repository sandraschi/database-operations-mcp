    ...
    requiredFields: [
      {
        ...
        label: "Host",
        description:
          "Machine name or IP where PostgreSQL is running. Use localhost or 127.0.0.1 for a server on the same machine.",
      },
      {
        key: "port",
        label: "Port",
        description:
          "TCP port. The default is 5432. Your DBA or hosting provider will confirm if it is different.",
      },
      {
        key: "user",
        label: "User",
        description:
          "Database user name. Must have permission to connect and to access the databases you need.",
      },
      {
        key: "password",
        label: "Password",
        description:
          "Password for that user. Leave blank only if the server is configured to allow passwordless access (e.g. local trust).",
      },
      {
        key: "database",
        label: "Database",
        description:
          "Name of the specific database (catalog) to connect to. A single PostgreSQL server can host many databases.",
      },
    ],
    optionalFields: [
      {
        ...
        label: "SSL mode",
        description:
          "Use 'require' or 'verify-full' for encrypted connections; 'prefer' or 'disable' for local/dev. Default is often 'prefer'.",
      },
    ],
    examples: [
    ...
    requiredFields: [
      {
        ...
        label: "Host",
        description:
          "Machine name or IP of the MySQL server. Use localhost for the same machine.",
      },
      { key: "port", label: "Port", description: "TCP port. Default is 3306." },
      {
        key: "user",
        label: "User",
        description: "MySQL user name with connect and usage rights.",
      },
      {
        key: "password",
        label: "Password",
  ...
  },
  {
    ...
    requiredFields: [
      ...
      { key: "port", label: "Port", description: "Default is 27017." },
      {
        key: "database",
        label: "Database",
        description: "Name of the database (top-level namespace) to use.",
      },
    ],
    optionalFields: [
      {
    ...
    optionalFields: [
      {
        ...
        label: "Password",
        description:
          "Required if the server has requirepass set. Leave blank for no auth.",
      },
    ],
    examples: [
    ...
    optionalFields: [
      {
        ...
        label: "API key",
        description:
          "For LanceDB cloud; obtain from the LanceDB cloud console.",
      },
      {
        key: "region",
        label: "Region",
    ...
    optionalFields: [
      {
        key: "host",
        label: "Host",
        description: "For client/server mode; ChromaDB server host.",
      },
      {
        key: "port",
        label: "Port",
        description: "Port for the ChromaDB server.",
      },
    ],
    examples: [

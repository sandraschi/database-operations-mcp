...
import {
  DATABASE_TYPES_HELP,
  type DatabaseTypeHelp,
} from "@/common/database-types-help";
import {
  Card,
  CardContent,
  CardDescription,
...
export function DatabaseTypesHelp() {
  const [activeType, setActiveType] = useState(() =>
    resolveTypeId(window.location.hash),
  );

  useEffect(() => {
    const onHashChange = () =>
      setActiveType(resolveTypeId(window.location.hash));
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">
          What each type is, when to use it, and how to connect
        </p>
      </div>

...
export function Schema() {
  ...
  const [describe, setDescribe] = useState<Record<string, unknown> | null>(
    null,
  );
  ...
  const loadActiveAndList = () => {
    if (!connectionName.trim()) {
      setError(
        "Enter a connection name (or use Connections page to set active).",
      );
      return;
    }
    setError(null);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            List databases, tables, and inspect columns
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              {selectedTable
                ? `Columns and metadata for ${selectedTable}`
                : "Select a table"}
            </CardDescription>
          </CardHeader>
          <CardContent>

...
import { Activity, Shield, Network, HardDrive, Wrench } from "lucide-react";
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">Database-operations MCP Dashboard</h2>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-slate-200">Service Status</CardTitle>
                        <Shield className={cn("h-4 w-4", healthy ? "text-emerald-500" : "text-red-500")} />
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="col-span-4 border-slate-800 bg-slate-950/50">
                    ...
                    <CardContent>
                        <div className="h-[200px] font-mono text-xs p-4 overflow-y-auto border border-slate-800 rounded-md bg-slate-900/50 text-slate-400 space-y-1">
                            {loading && <p className="text-blue-400">[system] Checking backend...</p>}
                            {healthy && <p className="text-emerald-400">[success] Backend healthy, MCP mounted at /mcp</p>}
                            {healthy && <p>[network] API /api/tools, /api/tools/call reachable.</p>}
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center">
                                ...
                                <div className="ml-2 space-y-1">
                                    ...
                                    <p className="text-xs text-slate-400">Single process, MCP + REST</p>
                                </div>
                            </div>
                            <div className="flex items-center">
                                <Activity className={cn("h-4 w-4 mr-2", healthy ? "text-emerald-500" : "text-slate-500")} />
                                <div className="ml-2 space-y-1">
                                    <p className="text-sm font-medium leading-none text-white">Heartbeat</p>
                                    <p className="text-xs text-slate-400">{healthy ? "Nominal" : "No response"}</p>
                                </div>
                            </div>
                        </div>
                    </CardContent>

...
import { CheckCircle2, Activity, Zap, AlertTriangle } from "lucide-react";
...
export function Health() {
    ...
    const listMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            const list = (d?.connections as { name?: string; connection_name?: string }[]) ?? [];
    ...
    const healthy = status === "ok" || status === true || healthResult?.success === true;

    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">Health &amp; Diagnostics</h2>
                ...
                <div className="flex gap-2">
                    ...
                    <Button
                        ...
                        onClick={runScan}
                        disabled={!connectionName.trim() || healthMutation.isPending || metricsMutation.isPending}
                    >
                        <Zap className="mr-2 h-4 w-4" />
                        {healthMutation.isPending || metricsMutation.isPending ? "Running..." : "Run health check"}
                    </Button>
                </div>
            </div>
                    ...
                    <Card className="border-slate-800 bg-slate-950/50 backdrop-blur-sm">
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                                {healthResult?.message as string ?? (healthy ? "Connection OK" : "See details below")}
                            </p>
                        </CardContent>
                    </Card>
                    <Card className="border-slate-800 bg-slate-950/50 backdrop-blur-sm">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium text-slate-400">Metrics</CardTitle>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-slate-500 mt-1">Performance and usage</p>
                        </CardContent>
                    </Card>
                </div>
            )}
            ...
            <div className="grid gap-6 md:grid-cols-2">
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader>
                        <CardTitle className="text-slate-200">Health check result</CardTitle>
                        <CardDescription className="text-slate-500">database_health_check output</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader>
                        ...
                        <CardDescription className="text-slate-500">get_database_metrics output</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <ScrollArea className="h-[200px] rounded border border-slate-800">
                            <pre className="p-3 text-xs text-slate-300 whitespace-pre-wrap break-all font-mono">

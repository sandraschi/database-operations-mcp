...
export function ConnectionWizard() {
  ...
  const registerMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setSuccess(
        (d?.message as string) ??
          "Connection registered. Set it active on the Connections page.",
      );
    },
    ...
    sqlite: [
      {
        label: "Database path",
        key: "database",
        placeholder: "C:/data/mydb.db or ./local.db",
      },
    ],
    postgresql: [
    ...
    duckdb: [
      {
        label: "Database path",
        key: "database",
        placeholder: "C:/data/file.duckdb or :memory:",
      },
    ],
    mongodb: [
      { label: "Host", key: "host", placeholder: "localhost" },
    ...
    lancedb: [
      {
        label: "URI",
        key: "uri",
        placeholder: "./data/lancedb or db://project",
      },
  ...
  const fields =
    configFields[databaseType as keyof typeof configFields] ??
    configFields.sqlite;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400">
          Guided setup: name, type, and connection details
        </p>
      </div>
        ...
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader>
            <CardTitle className="text-white">
              Step {step}: {step === 1 ? "Name and type" : "Connection details"}
            </CardTitle>
              ...
              <>
                ...
                {typeHelp && (
                  <div className="rounded-md border border-slate-700 bg-slate-900/50 overflow-hidden">
                    ...
                    >
                      <span>
                        Help: {typeHelp.name} — {typeHelp.shortDescription}
                      </span>
                      {showHelp ? (
                ...
                {typeHelp && (
                  <div className="rounded-md border border-slate-700 bg-slate-900/50 px-4 py-3 text-sm">
                    ...
                    <p className="text-slate-400 mb-2">
                      {typeHelp.shortDescription}
                    </p>
                    <p className="text-slate-500 text-xs">
                      Required:{" "}
                      {typeHelp.requiredFields.map((f) => f.label).join(", ")}
                    </p>
                  </div>
                )}
                {fields.map((f) => (

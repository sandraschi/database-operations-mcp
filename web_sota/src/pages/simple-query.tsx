...
export function SimpleQuery() {
    ...
    const listTablesMutation = useMutation({
        mutationFn: (conn: string) =>
            callTool("db_schema", { operation: "list_tables", connection_name: conn }),
        onSuccess: (data) => {
            setError(null);
    ...
    const describeMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            const names = cols.map((c: unknown) =>
    ...
    const runMutation = useMutation({
        ...
        onSuccess: (data) => {
            setError(null);
            const d = extractData(data?.result);
            const list = (d?.rows as Record<string, unknown>[]) ?? (d?.result as Record<string, unknown>[]) ?? [];
    ...
    const runQuery = () => {
        ...
        if (filterCol && filterVal.trim() !== "") {
            ...
            const val = filterOp === "LIKE" ? `'%${filterVal.replace(/'/g, "''")}%'` : `'${String(filterVal).replace(/'/g, "''")}'`;
    ...
    const displayCols = columns.length > 0 ? columns : (rows.length > 0 ? Object.keys(rows[0] as object) : []);

    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-2xl font-bold tracking-tight text-white">Simple query</h2>
                <p className="text-slate-400">Pick a table and columns, add an optional filter — no SQL needed</p>
            </div>

            <Card className="border-slate-800 bg-slate-950/50">
                <CardHeader>
                    <CardTitle className="text-white">Connection and table</CardTitle>
                    <CardDescription className="text-slate-400">Load tables, then pick one to choose columns.</CardDescription>
                </CardHeader>
                <CardContent className="flex flex-wrap items-end gap-4">
                    ...
                    {tables.length > 0 && (
                        <>
                            ...
                            {columns.length > 0 && (
                                <div className="grid gap-2">
                                    <Label className="text-slate-300">Columns (click to toggle)</Label>
            ...
            {selectedTable && columns.length > 0 && (
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader>
                        ...
                        <CardDescription className="text-slate-400">Optional: one filter and max rows.</CardDescription>
                    </CardHeader>
                    <CardContent className="flex flex-wrap items-end gap-4">
                        <div className="grid gap-2">
                        ...
                        >
                            {runMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="mr-2 h-4 w-4" />}
                            Run query
                        </Button>
                    </CardContent>
                </Card>
            ...
            {rows.length > 0 && (
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader>
                        <CardTitle className="text-white">Result ({rows.length} rows)</CardTitle>
                    </CardHeader>
                    <CardContent>
                                ...
                                <table className="w-full border-collapse text-sm">
                                    <thead>
                                        <tr className="border-b border-slate-700 bg-slate-900/50">
                                            {displayCols.map((col) => (
                                                <th key={col} className="text-left px-4 py-2 font-medium text-slate-300 whitespace-nowrap">{col}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>

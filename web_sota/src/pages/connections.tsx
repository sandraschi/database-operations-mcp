...
import { Database, CheckCircle2, Loader2, RefreshCw } from "lucide-react";
...
export function Connections() {
    ...
    const [supported, setSupported] = useState<Record<string, unknown> | null>(null);
    const [listResult, setListResult] = useState<Record<string, unknown> | null>(null);
    ...
    const supportedMutation = useMutation({
        mutationFn: () => callTool("db_connection", { operation: "list_supported" }),
        onSuccess: (data) => {
            setError(null);
    ...
    const activeName = (activeResult?.connection_name as string) ?? (activeResult?.active as string) ?? null;
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">Connections</h2>
                    <p className="text-slate-400">Registered databases and active connection</p>
                </div>
                <div className="flex gap-2">
                    ...
                    >
                        <RefreshCw className={cn("mr-2 h-4 w-4", (listMutation.isPending || activeMutation.isPending) && "animate-spin")} />
                        Refresh
                    </Button>
                </div>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader>
                        ...
                        <CardDescription className="text-slate-400">
                            {connections.length} connection(s). Set active for Schema / Data pages.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    return (
                                        <li
                                            ...
                                            className={cn(
                                                ...
                                                isActive ? "bg-slate-800/80 text-white" : "text-slate-300 hover:bg-slate-800/50"
                                            )}
                                        >
                                            <span className="font-mono truncate">{name}</span>
                                            <div className="flex items-center gap-1 shrink-0">
                                                {isActive && (
                                                    <Badge className="bg-emerald-900/50 text-emerald-400 border-0 text-xs">Active</Badge>
                                                )}
                                                <Button
                                                    size="sm"
                                                    variant="ghost"
                                                ...
                                                >
                                                    {testingThis ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle2 className="h-3 w-3" />}
                                                </Button>
                                                <Button
                                                    size="sm"
                                                    variant="ghost"
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader>
                        <CardTitle className="text-white">Supported database types</CardTitle>
                        <CardDescription className="text-slate-400">
                    ...
                    <CardContent>
                        {supported && (supported.types as unknown[])?.length > 0 ? (
                            ...
                            <p className="text-slate-500 text-sm">Click &quot;Supported types&quot; to load.</p>
                        )}
                    </CardContent>
                </Card>
            </div>

...
export default function DatabaseBrowser() {
  ...
  const inspectDb = async () => {
    ...
    try {
      ...
      setError(
        "Failed to communicate with DB engine. Ensure MCP server is running.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const loadTable = async (tableName: string) => {
    ...
    try {
      // @ts-expect-error
      const dataResult = await window.mcp.callTool(
        "database-operations-mcp",
      ...
      // @ts-expect-error
      const schemaResult = await window.mcp.callTool(
        "database-operations-mcp",
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-slate-400 mt-1 italic">
            Industrial-grade arbitrary SQLite explorer
          </p>
        </div>
      </div>
                ...
                {filteredTables.map((table) => (
                  ...
                  >
                    <div className="truncate">
                      <div className="font-medium truncate group-hover:text-slate-200">
                        {table.table_name}
                      </div>
                    ...
                    <thead className="sticky top-0 z-10 bg-slate-900 shadow-sm border-b border-slate-700">
                      <tr>
                        {columns.map((col) => (
                          <th
                            key={col}
                            className="px-4 py-3 font-bold text-slate-300 uppercase tracking-tighter whitespace-nowrap bg-slate-800/80"
                          >
                            {col}
                          </th>
                        ))}
                      ...
                      {tableData.map((row, i) => (
                        ...
                        >
                          {columns.map((col) => (
                            <td
                              key={`${i}-${col}`}
                              className="px-4 py-2.5 text-slate-400 font-mono truncate max-w-[200px]"
                            >
                              {row[col]?.toString() ?? (

...
export function ExportImport() {
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const d = extractData(data?.result);
      const total =
        (d?.total_inserted as number) ?? (d?.rows_affected as number);
  ...
  const downloadAsJson = () => {
    ...
    downloadBlob(
      JSON.stringify(exportResult.rows, null, 2),
      "export.json",
      "application/json",
    );
  };
  ...
  const downloadAsCsv = () => {
    ...
    downloadBlob(
      rowsToCSV(exportResult.rows, exportResult.columns),
      "export.csv",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400">
          Export query results (JSON/CSV download) and import data via
          batch_insert
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              Run a query (max 10k rows), then download as JSON or CSV. Uses
            ...
            {exportResult && (
              <div className="space-y-2">
                <p className="text-sm text-slate-400">
                  {exportResult.rows?.length ?? 0} rows
                </p>
        ...
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              Paste JSON array of objects or upload JSON/CSV; inserts via
              db_operations batch_insert.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              <Label className="text-slate-300">
                JSON array (optional if using file)
              </Label>
              <textarea

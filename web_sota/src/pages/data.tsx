...
export function Data() {
    ...
    const sampleMutation = useMutation({
        mutationFn: ({ conn, table, limit }: { conn: string; table: string; limit: number }) =>
            callTool("db_operations", {
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <Button
                            ...
                            onClick={runQuery}
                            disabled={queryMutation.isPending || !connectionName.trim() || !query.trim()}
                        >
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <Button
                            ...
                            onClick={runSample}
                            disabled={sampleMutation.isPending || !connectionName.trim() || !tableName.trim()}
                        >
                ...
                <CardContent>
                    <ScrollArea className="h-[320px] rounded border border-slate-800 bg-slate-900/30">
                        <pre className="p-4 text-xs text-slate-300 whitespace-pre-wrap break-all font-mono">
                            {result != null ? JSON.stringify(result, null, 2) : "Run a query or sample to see results."}
                        </pre>
                    </ScrollArea>
                </CardContent>
            </Card>
